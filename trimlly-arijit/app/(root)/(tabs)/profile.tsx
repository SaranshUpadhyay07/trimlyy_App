...
// Importing the useClerk hook to manage authentication
...
const Profile = () => {
  ...
  // };
    ...
    const { signOut } = useClerk(); // make sure this is defined
    
    const handleSignOut = async () => {
      try {
        await signOut(); // Clerk clears session
        router.replace("/(auth)/sign-in"); // ✅ clears navigation history
      } catch (err) {
  ...
  return (
    <SafeAreaView className="bg-[#f8f4eb] h-full">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32, paddingHorizontal: 7 }}>
        ...
        {/* Logout Option */}
        <View className="flex flex-col mt-5 pt-5 border-primary-200 border-t">
          <SettingsItem icon={icons.logout} title="Logout" textStyle="text-danger" showArrow={false} onPress={handleSignOut} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
